...
with open("/Users/alexoh/Documents/GitHub/arrastools/««««« CORE »»»»»/rollbotlog.txt", "w+") as log:
    ...
    if simulate and simulate_count > 0:
        ...
        for sim_i in range(simulate_count):
            ...
            if rarity_value >= 1e15:
                log.write(f"[SIM] Rolled {quantity} × T{tier} | R%: 1 in {format_rarity(chance)} | {gained} ★" + "\n")
            else:
                log.write(f"[SIM] Rolled {quantity} × T{tier} | R%: {round(chance, 3)}% (1 in {format_rarity(chance)}) | {gained} ★" + "\n")
    ...
    while True:
        ...
        if rarity_value >= 1e15:
            ...
            info = f"{combo_display} | R%: {round(chance, 3)}% (1 in {format_rarity(chance)}) | {round(gained, 2)} ★{bonus_display}"
