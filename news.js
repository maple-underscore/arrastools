...
// Compress multi-line text entries into single lines
const allText = news.map(entry => {
  // Replace newlines with spaces and compress multiple spaces
  return entry.text.replace(/\s+/g, ' ').trim();
}).join('\n');

const outputPath = join(__dirname, 'copypastas', 'ad_news.txt');

writeFileSync(outputPath, allText, 'utf8');
console.log(`Wrote ${news.length} entries to ${outputPath}`);